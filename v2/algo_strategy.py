...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def init_build(self, state:gamelib.GameState):
        for x in self.init_turret_points:
            state.attempt_spawn(TURRET, [x[0], x[1]])
        for x in self.init_wall_points:
            state.attempt_spawn(WALL, [x[0], x[1]])        
    ...
    def on_turn(self, turn_state):
        ...
        if game_state.turn_number == 0:
            ...
            for i in range(self.later_walls_built): #building the extra walls
                for x in self.later_wall_points[i]:
                    game_state.attempt_spawn(WALL, [x[0], x[1]])
                    sp -= 0.5
                    if sp < 0.5: break
                if sp < 0.5: break
            ...
            if sp > 0.5:
                for x in range(self.later_walls_built, 3):
                    ...
                    for y in self.later_wall_points[x]:
                        game_state.attempt_spawn(WALL, [y[0], y[1]])
                        sp -= 0.5
                        if sp < 0.5: break
                    
                    if sp < 0.5: break

            game_state.attempt_upgrade(self.wall_upgrades)

            game_state.attempt_upgrade(self.impt_turret_side)

            

            for x in self.later_turret_points:
                game_state.attempt_spawn(TURRET, [x[0], x[1]])

            game_state.attempt_upgrade(self.impt_turret_mid)

            for x in self.last_wall_points:
                game_state.attempt_spawn(WALL, [x[0], x[1]])
            
            game_state.attempt_upgrade(self.impt_turret_mid_outer)
            ...
            # for x in self.wall_upgrades:
            #     game_state.attempt_upgrade([x[0], x[1]])
                # sp -= 1.5
                # if sp < 1.5: break
